...
1 초	512 MB	35920	6065	3839	19.667%
문제
...
int rule[1001][1001];
int consTime[1001];
int calcTime(int N, int target) {
	int needBase = 0, max = 0;
	for (int i = 1; i <= N;i++) {
		if (rule[i][target]) {
			needBase++;
		}
	...
	for (int i = 1; i <= N;i++) {
		int temp = -1;
		if (rule[i][target]) {
			temp = (consTime[i] == -1) ? calcTime(N, i) : consTime[i];
		}
		if (temp > max) max = temp;
	}
	consTime[target] = max + D[target];
	return max + D[target];
}
int main() {
	int T = 0;
	...
	while (T--) {
		int N, K, W;
		memset(consTime, -1, sizeof(consTime));
		memset(D, -1, sizeof(D));
